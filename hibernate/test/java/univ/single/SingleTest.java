...
public class SingleTest {
	...
	public void update() {
		...
		single.setName("ddd");
		/**
		 * 和save一样，此时会执行一条select语句，用与执行真正的update语句之前比较一下，如果有变化则在session清理缓存时执行真正的update语句，否则不执行
		 * select single0_.id as id1_4_0_, single0_.name as name2_4_0_, single0_.age as age3_4_0_, 
		 * single0_.date as date4_4_0_ * from T_Single single0_ where single0_.id=?
		
		 */
		session.update(single);//和save一样，此时会执行一条select语句，select single0_.id as id1_4_0_, single0_.name as name2_4_0_, single0_.age as age3_4_0_, single0_.date as date4_4_0_ from T_Single single0_ where single0_.id=?
